...
export default function UserTable() {
    ...
    const columns =  [
        ...
        { 
            id: 'authpress-2fa-provider',
            name: '2FA Type',
            selector: row => row.data.user_meta['authpress-2fa-provider']?row.data.user_meta['authpress-2fa-provider']:'N/A',
            sortable: true,
        },
        { 
            id: 'action',
    ...
    */
    ...
    const handleClick2faReset = (user_id) => {
        ...
        const confirmed = window.confirm(__('Are you sure you want to proceed?', 'authpress'));
        if (confirmed) {
            apiService.formDataPost('authpress_user_2fa_reset',{"user_id": user_id})
            .then(data=> {
    ...
    const handleClick2faResetAll = () => {
        const confirmed = window.confirm(__('Are you sure you want to proceed?', 'authpress'));
        if (confirmed) {
            apiService.formDataPost('authpress_user_2fa_reset_all', {})
            .then(data=> {
                console.log(data)
                // setForceLogoutLoading(false)            
                if(data.success){
    ...
    const handleBulkActionSubmit = () => {
        if (selectedRows.length) {
            ...
            if (bulkAction === "reset") {
                ...
                apiService.formDataPost('authpress_user_2fa_reset_users',{"user_ids": ids})
                .then(data=> {
                    console.log(data)
                    // setForceLogoutLoading(false)            
                    if(data.success){
                ...
                // apiService.formDataPost('authpress_send_password_reset_emails',{"user_ids": ids})
                // .then(data=> {
                //     // setForceLogoutLoading(false)            
                //     if(data.success){
                //         setBulkAction("-1")
            ...
            <div className="row justify-content-between mb-30">
                <div className="col-8 filters">
                    ...
                    <div className="bulk-action-wrapper">
                        <select value={bulkAction} name="action"  onChange={handleBulkActionChange}>
                            <option value="-1">{__( 'Bulk actions', 'authpress' )}</option>
                            <option value="reset">{__( 'Reset', 'authpress' )}</option>
                            <option value="delete">{__( 'Delete', 'authpress' )}</option>
                        </select>
                ...
                <div className="col-auto">
                    <div className="search-unit d-flex gap-2 align-items-center">
                        ...
                        <button className="button button-secondary" type="button">{__( 'Search', 'authpress' )}</button>
                    </div>
                    <div className="action-buttons d-flex align-items-center">
                        <span onClick={handleClick2faResetAll}>{__( 'Reset All 2FA', 'authpress' )}</span>
                        <span onClick={handleClick2faDeleteAll}>{__( 'Delete All 2FA', 'authpress' )}</span>
                    </div>
                </div>
            </div>            
