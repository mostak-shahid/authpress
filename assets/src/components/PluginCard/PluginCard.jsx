...
export default function PluginCard(plugin) {
    ...
    */
    ...
    const checkPluginStatus = useCallback(async () => {
        ...
        setErrorMessage("");
        try {
            const result = await formDataPost('authpress_ajax_plugins_status', {
                file:plugin_file,
            });
    ...
    const getButtonLabel = () => {
        switch (pluginStatus) {
            case "checking":
                return __("Checking...", "authpress");
            case "not_installed":
                return __("Install Now", "authpress");
            case "installed":
                return __("Activate", "authpress");
            case "installing":
                return __("Installing...", "authpress");
            case "installation_complete": // New state
                return __("Installed", "authpress");
            case "activating":
                return __("Activating...", "authpress");
            case "activated":
                return __("Activated", "authpress");
            case "error":
                return __("Try Again", "authpress");
            default:
                return __("Install Now", "authpress");
        }
    };
    ...
    return (
        <div className="authpress-plugin-card p-4">
            <Space align='center'>
                <img
                    alt={name}
                    src={image}
            ...
            <div className="flex justify-between mt-1">
                <Space align='center'><IconHistogram style={{fontSize: 24}} /><span>{__(`${active_installs} ${active_installs>0?"+":""} active installations`, "authpress")}</span></Space>
                <Space align='center'><WordPress width='24' height='24'/> {__(`Tested with ${tested}`, "authpress")}</Space>
            </div>
