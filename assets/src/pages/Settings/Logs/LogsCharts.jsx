    ...
    const fetchChartsData = async () => {
        ...
        try {
            const [overTime, byCategory, topUsers, topIps, hourlyActivity] = await Promise.all([
                apiFetch({ path: '/authpress/v1/logs/stats/over-time' }),
                apiFetch({ path: '/authpress/v1/logs/stats/by-category' }),
                apiFetch({ path: '/authpress/v1/logs/stats/top-users' }),
                apiFetch({ path: '/authpress/v1/logs/stats/top-ips' }),
                apiFetch({ path: '/authpress/v1/logs/stats/hourly-activity' }),
            ]);
    ...
    return (
        <div>
            <Spin spinning={chartsLoading}>
                <Row gutter={16}>
                    <Col xs={24} md={12}>
                        <Card style={{ marginBottom: 16 }} bodyStyle={{padding: 0}} title={__('Logs Over Time', 'authpress')} headerLine={true}>
                            <VChart spec={overTimeSpec} />
                        </Card>
                    </Col>
                    <Col xs={24} md={12}>
                        <Card style={{ marginBottom: 16 }} bodyStyle={{padding: 0}} title={__('Logs by Category', 'authpress')} headerLine={true}>
                            <VChart spec={byCategorySpec} />
                        </Card>
                    </Col>
                    <Col xs={24} md={12}>
                        <Card style={{ marginBottom: 16 }} bodyStyle={{padding: 0}} title={__('Top Users', 'authpress')} headerLine={true}>
                            <VChart spec={topUsersSpec} />
                        </Card>
                    </Col>
                    <Col xs={24} md={12}>
                        <Card style={{ marginBottom: 16 }} bodyStyle={{padding: 0}} title={__('Category Share', 'authpress')} headerLine={true}>
                            <VChart spec={categoryPieSpec} />
                        </Card>
                    </Col>
                    <Col xs={24} md={12}>
                        <Card style={{ marginBottom: 16 }} bodyStyle={{padding: 0}} title={__('Top IPs', 'authpress')} headerLine={true}>
                            <VChart spec={topIpsSpec} />
                        </Card>
                    </Col>
                    <Col xs={24} md={12}>
                        <Card style={{ marginBottom: 16 }} bodyStyle={{padding: 0}} title={__('Hourly Activity', 'authpress')} headerLine={true}>
