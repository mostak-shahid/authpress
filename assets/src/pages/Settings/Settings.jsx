...
const Settings = () => {
    ...
    const handleSubmit = async (section, values) => {
        try {
            const result = await apiFetch({
                path: "/authpress/v1/options",
                method: 'POST',
                data: { authpress_options: { ...settings, [section]: values } }
            });
            if (result.success) {
    ...
    const handleReset = async (section) => {
        try {
            const result = await apiFetch({
                path: "/authpress/v1/options/reset-settings",
                method: 'POST',
                data: { name: section }
            });
            if (result.success) {
                ...
                Notification.error({
                    title: __("Error", "authpress"),
                    content: __("Error resetting settings. Please try again.", "authpress"),
                    duration: 3,
                    position: 'topRight',
                });
            }
    ...
    useEffect(() => {
        if (authpress_ajax_obj?.isPro) {
            import("authpresspro/MenuItems")
                .then((mod) => {
    ...
    useEffect(() => {
        if (authpress_ajax_obj?.extraMenuItems) {
            setRemoteItems(authpress_ajax_obj.extraMenuItems);
        }
    }, []);
        ...
        <>
            ...
            <Nav 
                items= {[
                    {
                        itemKey: "vip",
                        text: __("VIP Priority Support", "authpress"),
                        icon: <IconSend />,
                        link: "https://wordpress.org/support/plugin/authpress/",
                        linkOptions: {
                            target: '_blank',
                    ...
                    {
                        itemKey: "help",
                        text: __("Help Center", "authpress"),
                        icon: <IconHelpCircle />,
                        link: "https://mostak-shahid.github.io/plugins/authpress.html",
                        linkOptions: {
                            target: '_blank',
                            rel: 'noopener noreferrer', // recommended for security
                        },
                    ...
                    {
                        itemKey: "community",
                        text: __("Join the Community", "authpress"),
                        icon: <IconUserAdd />,
                    ...
                    {
                        itemKey: "rate",
                        text: __("Rate Us", "authpress"),
                        icon: <IconLikeHeart />,
                        link: "https://wordpress.org/support/plugin/authpress/reviews/",
                        linkOptions: {
                            target: '_blank',
                            rel: 'noopener noreferrer', // recommended for security
                        },
