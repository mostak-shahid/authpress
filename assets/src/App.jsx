...
export default function App() {
    ...
    //         try {
    //             const response = await apiFetch({
    //                 path: '/authpress/v1/get-option?option_name=mospress_read_news',
    //                 method: 'GET'
    //             });
    ...
    return (
        <LocaleProvider locale={en_US}>
            <div className="authpress-settings-container" style={{backgroundColor: 'var(--semi-color-bg-1)'}}>
                {!authpress_ajax_obj?.isPro &&
                    <Banner 
                    ...
                    <HorizontalMenuControl
                        ...
                        footerContent = {(
                            <Space className="header-menu-footer-content" align='center'>  
                                ...
                                {/* <a
                                    href="https://wordpress.org/support/plugin/authpress/"
                                    target="_blank"
                                ...
                                <a
                                    href="https://wordpress.org/support/plugin/authpress/reviews/"
                                    target="_blank"
                                    rel="noreferrer noopener"
                                    aria-label={ __(
                                ...
                                <Button 
                                    ...
                                    aria-label={__("Documentation", 'authpress')}
                                    onClick={ () =>
                                        window.open(
                                            'https://wordpress.org/support/plugin/authpress/',
                                            '_blank'
                                        )
                                    }
                                />
                                <Button 
                                    theme='outline' 
                                    icon={<IconStar />} 
                                    aria-label={__("Help Center", 'authpress')} 

                                    onClick={ () =>
                                        window.open(
                                            'https://wordpress.org/support/plugin/authpress/reviews/',
                                            '_blank'
                                        )
                                    }
                                />
                ...
                >
                    <Row type="flex" gutter={24} align="middle" justify="space-between">
                        <Col xs={24} lg={12} className="text-center lg:text-left mb-2 lg:mb-0">
                            <Text>{__(`Copyright © ${year} `, 'authpress')}</Text>
                            <Text link={{ href: Details?.authorURI, target: '_blank' }}>{Details?.author}. </Text>
                            <Text>{__(`All Rights Reserved.`, 'authpress')}</Text>
                        ...
                        <Col xs={24} lg={12} className="text-center lg:text-right">
                            <Space align='center' spacing='medium'>
                                {authpress_ajax_obj?.isPro === '1'?
                                    <>
                                        <Badge count={__( 'Pro', "authpress" )} theme='light' style={{padding: 8, height: 'auto'}} />
                                        <Badge count={authpress_ajax_obj?.proVersion} theme='light' style={{padding: 8, height: 'auto'}} />
                                    </>
                                    :
                                    <>                                    
                                        <Badge count={__( 'Free', "authpress" )} theme='light' style={{padding: 8, height: 'auto'}} />
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                            ...
                            {Math.ceil(newsItems.length / itemsPerPage) > 1 && (
                                <div style={{ borderTop: '1px solid var(--semi-color-border)', flexShrink: 0 }} className='flex justify-between items-center py-4 mt-4'>
                                    ...
                                    >
                                        {__("Previous", "authpress")}
                                    </Button>
                                    <Text>
                                        {__("Page", "authpress")} {newsCurrentPage} {__("of", "authpress")} {Math.ceil(newsItems.length / itemsPerPage)}
                                    </Text>
                                    <Button
                                        size="small"
                                        onClick={() => setNewsCurrentPage(newsCurrentPage + 1)}
