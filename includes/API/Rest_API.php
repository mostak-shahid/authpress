...
class Rest_API
{
    ...
    public function get_product_specifications( WP_REST_Request $request ) {
        ...
        if ( ! $product_id ) {
            return new WP_Error(
                'invalid_product_id',
                __( 'Invalid product ID', 'authpress' ),
                array( 'status' => 400 )
            );
        }
    ...
    public function save_product_specifications( WP_REST_Request $request ) {
        ...
        if ( ! isset( $data['specifications'] ) ) {
            return new WP_Error(
                'missing_data',
                __( 'Missing specifications data', 'authpress' ),
                array( 'status' => 400 )
            );
        }
        ...
        return rest_ensure_response( array(
            'success' => true,
            'message' => __( 'Specifications saved successfully', 'authpress' ),
        ) );
    }
    /**
    ...
    public function check_permission( WP_REST_Request $request ) {
        ...
        if ( ! current_user_can( 'manage_options' ) ) {
            return new WP_Error(
                'rest_forbidden',
                __( 'You do not have permission to access this endpoint.', 'authpress' ),
                array( 'status' => 403 )
            );
        }
	...
	public function update_settings(WP_REST_Request $request) //WP_REST_Request $request
	{
		...
		LogsController::log_settings_change($authpress_options_old, $authpress_options);
    ...
    public function reset_settings_all(WP_REST_Request $request)
	{
		...
		$response = [
			'success' => true,
			'message'	=> esc_html__('Settings reset successfully.', 'authpress')
		];
    ...
    public function import_settings(WP_REST_Request $request)
	{
        ...
        LogsController::log_settings_change($authpress_options_old, $data);

        return rest_ensure_response(array(
            'success' => true,
            'message' => esc_html__('Settings imported successfully.', 'authpress')
        ));
    }
                                ...
                                <tr>
                                    <td style="background-color: #f8f9fa; padding: 20px; text-align: center; border-top: 1px solid #e0e0e0;">
                                        <p style="color: #999999; font-size: 12px; margin: 0;">
                                            <?php esc_html__('This email was sent from', 'authpress'); ?> 
                                ...
                                <tr>
                                    <td style="padding: 40px 30px;">
                                        ...
                                        <p style="color: #333333; font-size: 14px; line-height: 1.6; margin: 0;">
                                            <strong>Plugin:</strong> AuthPress
                                        </p>
                                    </td>
                                </tr>
                                <tr>
    ...
    {
        $webhook_url = apply_filters('authpress_feedback_webhook_url', '');
        ...
        if (is_wp_error($response)) {
            error_log('AuthPress - Failed to send feedback to webhook: ' . $response->get_error_message());
            return false;
        }
    ...
    public function rest_set_settings_theme(WP_REST_Request $request)
    {
        ...
        $response = [
            'success' => true,
            'msg' => esc_html__('Theme set successfully.', 'authpress'),
        ];
	...
	public function rest_set_option(WP_REST_Request $request)
	{
		...
		if (empty($option_name)) {
			return new WP_Error(
				'rest_invalid_param',
				__('Option name is required.', 'authpress'),
				array('status' => 400)
			);
		}
		...
		if ($updated === false) {
			return new WP_Error(
				'rest_update_failed',
				__('Failed to update option.', 'authpress'),
				array('status' => 500)
			);
		}
		...
		$response = [
			'success' => true,
			'msg' => esc_html__('Option updated successfully.', 'authpress'),
		];
    ...
    //     $encrypted_key = get_option( 'authpress_deactive_key' );

    //     if ( false === $encrypted_key ) {
    //         return new WP_Error(
    //             'key_not_found',
    //             __( 'Deactivation key not found. Please reactivate the plugin.', 'authpress' ),
    //             array( 'status' => 404 )
    //         );
    //     }
    ...
    //         return new WP_Error(
    //             'decryption_failed',
    //             __( 'Failed to decrypt deactivation key. Please contact support.', 'authpress' ),
    //             array( 'status' => 500 )
    //         );
    //     }
    ...
    //             'message' => __( 'Deactivation link generated successfully.', 'authpress' ),
    //             'warning' => __( 'This link will only work once. After deactivation, a new link will be generated on reactivation.', 'authpress' ),
    //         ),
    //         200
    //     );
    // }
