...
class PublicClass
{
	...
	public function enqueue_styles()
	{
		...
		// wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/authpress-public.css', array(), $this->version, 'all' );
		wp_enqueue_style($this->plugin_name . '-public', AUTHPRESS_URL . 'public/css/public-style.css', array(), $this->version, 'all');
	}
	...
	public function enqueue_scripts()
	{
		...
		wp_enqueue_script($this->plugin_name . '-public-ajax', AUTHPRESS_URL . 'public/js/public-ajax.js', array('jquery'), $this->version, false);
		wp_enqueue_script($this->plugin_name . '-public-script', AUTHPRESS_URL . 'public/js/public-script.js', array('jquery'), $this->version, false);
		$ajax_params = array(
			'admin_url' => admin_url(),
			'ajax_url' => admin_url('admin-ajax.php'),
			'_wp_nonce' => esc_attr(wp_create_nonce('authpress_wp_nonce')),
			// 'install_plugin_wpnonce' => esc_attr(wp_create_nonce('updates')),
		);
		wp_localize_script($this->plugin_name . '-public-ajax', 'authpress_ajax_obj', $ajax_params);
	}
	public function authpress_ajax_callback()
	{
		if (isset($_POST['_wp_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_wp_nonce'])), 'authpress_wp_nonce')) {
