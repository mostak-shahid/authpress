...
class UserMeta {
    ...
    public function render_field($user) {
        ?>
        <h2><?php echo esc_html__('AuthPress Info', 'authpress'); ?></h2>
        <div id="authpress-profile-react-app"></div>
        <?php wp_nonce_field( 'mos_email_form_action', 'mos_email_form_field' ); ?>
        <table class="form-table">
            <tr>
                <th>
                    <label for="authpress_company">
                        <?php echo esc_html__('Company Name', 'authpress'); ?>
                    </label>
                </th>
                <td>
                    <input 
                        type="text" 
                        name="authpress_company" 
                        id="authpress_company"
                        value="<?php echo esc_attr(get_user_meta($user->ID, 'authpress_company', true)); ?>"
                        class="regular-text"
                    />
                    <p class="description">
                        <?php echo esc_html__('Enter the user\'s company name.', 'authpress'); ?>
                    </p>
                </td>
            </tr>
        </table>
    ...
    public function save_field($user_id) {
        ...
        if ( isset( $_POST['mos_email_form_field'] ) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['mos_email_form_field'])), 'mos_email_form_action' ) ) {
            $authpress_company = isset($_POST['authpress_company']) ? sanitize_text_field(wp_unslash($_POST['authpress_company'])) : '';            
            update_user_meta(
                $user_id,
                'authpress_company',
                $authpress_company
