...
class Action_Hook
{
    ...
    public function __construct()
	{
        ...
        add_action('admin_init', [$this, 'authpress_do_activation_redirect']);
        add_action('admin_menu', [$this, 'authpress_admin_menu']);
        add_action('current_screen', [$this, 'authpress_hide_admin_notices']);
        add_action('admin_head', [$this, 'authpress_option_form_submit']);
        add_action('upgrader_process_complete', [$this, 'authpress_update_completed'], 10, 2);
	...
	public function authpress_admin_menu()
	{
		add_menu_page(
			...
			AUTHPRESS_URL . 'admin/images/menu-icon.svg',
			57
		);
	}
	/**
	...
	public function authpress_dashboard_php_page_html()
	{
		...
		// include_once(AUTHPRESS_PATH . 'admin/partials/' . $this->plugin_name . '-admin-display.php');
		echo '<div id="authpress-settings-react-app"></div>';
	}
	public function authpress_dashboard_react_page_html()
	{
		...
		include_once(AUTHPRESS_PATH . 'admin/partials/' . $this->plugin_name . '-admin-display-react.php');
	}
	...
	public function authpress_update_completed($upgrader_object, $options)
	{
		...
		if ($options['action'] == 'update' && $options['type'] == 'plugin' && isset($options['plugins'])) {
			foreach ($options['plugins'] as $plugin) {
				...
				if ($plugin == plugin_basename(AUTHPRESS_MAIN_FILE)) {
					// do stuff here
					$authpress_options = array_replace_recursive(authpress_get_option(), get_option('authpress_options', []));
					update_option('authpress_options', $authpress_options);
				}
	...
	public static function authpress_deactivation_scripts()
	{
		?>
		<!-- Password Modal -->
		...
		<script type="text/javascript">
			jQuery(document).ready(function($) {
				...
				$('tr[data-plugin="authpress/authpress.php"] .deactivate a').on('click', function(e) {
					e.preventDefault();
